...
function sumOfCodes(str) {
  return Boolean(str) === true
    ? [...str].reduce((acc, value) => {
        return acc + +value.charCodeAt();
      }, 0)
    : 0;
}
...
function countVowels(str) {
  ...
  return [...str].reduce((acc, value) => {
    return vowels.includes(value) ? acc + 1 : acc;
  }, 0);
}
...
function encodeToRot13(str) {
  ...
  return [...str]
    .map((value) => {
      if (unicode.indexOf(value) === -1) return value;
      const index = unicode.indexOf(value);
      return rot13[index];
    })
    .join('');
}
